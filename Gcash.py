...
class Gcash:
    def __init__(self, root):
        ...
        def iRECORDDATA():
            ...
            if self.TRANSACTIONSTYPE.get() == "" or self.REFERENCENO.get() == "":
                messagebox.showerror("Error", "All fields are required")
            else:
                conn = mysql.connector.connect(host="localhost", user="root", password="", database="Gcash")
                cursor = conn.cursor()
                cursor.execute("Insert into TRANSACTIONS values ('" + TRANSACTIONSTYPE + "','" + MOBILENUMBER + "','" + AMOUNT + "','" + PAYMENTFEE + "','" + DATE + "','" + REFERENCENO + "')")
                conn.commit()
        ...
        def iDELETE():
            ...
            cursor.execute("DELETE FROM TRANSACTIONS WHERE REFERENCENO='" + REFERENCENO + "'")
            
            conn.commit()
            conn.close()
            fatch_data()
            messagebox.showinfo("Delete","Record has been deleted Succesfully")
